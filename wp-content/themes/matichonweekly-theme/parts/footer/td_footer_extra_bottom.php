...
$retina_footer_logo_width = ''; //Unixdev MOD: fix bug retina_footer_logo_width not defined
...
*/
$td_inline_logo = get_template_directory_uri() . '/images/matichonweekly-logo.png';
$buffy .= '<a href="' . esc_url(home_url( '/' )) . '"><img src="' . $td_inline_logo . '" alt="' . get_bloginfo( "name" ) . '" title="' . get_bloginfo( "name" ) . '"/></a>';    
$buffy .= '</aside></div>';
...
if(td_util::get_option('tds_footer_social') != 'no') {
	$buffy .= '<div class="td-pb-span4"><aside class="footer-social-wrap td-social-style-2">';
        ...
        //Unixdev MOD: truehits
        if ( "yes" == td_util::get_option( 'ud_truehits_enable' ) ) {
            $buffy .= '<div class="ud-truehits-wrapper">';
            $ud_truehits_code = td_util::get_option( 'ud_truehits_code' );
            $buffy .= stripslashes( $ud_truehits_code );
            $buffy .= '</div>';
        }
        //------------------
	$buffy .= '</aside></div>';
