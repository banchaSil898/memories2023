...
class td_block_ad_box extends td_block {
    ...
    function render_google_ads($ad_array, $atts) {
        ...
        if ($align == 'left') {
            ...
            $default_ad_sizes['content_inline_2'] = array (
                'm_w' => '300',  // big monitor - width
                'm_h' => '250',  // big monitor - height
    ...
    function render_ads($ad_array, $atts) {
        ...
        // Unixdev MOD: ---------------------------
        //init the disable variables
        $ud_ad_disable_m = false;
        $ud_ad_disable_tl = false;
        $ud_ad_disable_tp = false;
        $ud_ad_disable_p = false;

        if (!empty($ad_array['disable_m']) and $ad_array['disable_m'] == 'yes') {
            $ud_ad_disable_m = true;
        }

        if (!empty($ad_array['disable_tl']) and $ad_array['disable_tl'] == 'yes') {
            $ud_ad_disable_tl = true;
        }

        if (!empty($ad_array['disable_tp']) and $ad_array['disable_tp'] == 'yes') {
            $ud_ad_disable_tp = true;
        }

        if (!empty($ad_array['disable_p']) and $ad_array['disable_p'] == 'yes') {
            $ud_ad_disable_p = true;
        }
        // ------------------------------------------
	    ...
	    $buffy .= '<div class="td-a-rec td-a-rec-id-' . $spot_id . $align . ' '
            . (($ud_ad_disable_m === true) ? ' td-rec-hide-on-m' : '')
            . (($ud_ad_disable_tl === true) ? ' td-rec-hide-on-tl' : '')
            . (($ud_ad_disable_tp === true) ? ' td-rec-hide-on-tp' : '')
            . (($ud_ad_disable_p === true) ? ' td-rec-hide-on-p' : '')
            ...
            // Unixdev MOD ------------------------
            if ( ! empty( $ad_array['ud_real_ad_code'] ) ) {
                $buffy .= '<script type="text/javascript">' . "\n";
                    $buffy .= 'var ud_screen_width = document.body.clientWidth;' . "\n";
                    $buffy .= 'var '.$ad_array['is_visible_variable_name'].' = false';
                    if ($ud_ad_disable_m === false) {
                        $buffy .= ' || ( ud_screen_width >= 1140 )';
                    }

                    if ($ud_ad_disable_tl === false) {
                        $buffy .= ' || ( ud_screen_width >= 1019  && ud_screen_width < 1140 ) ';
                    }

                    if ($ud_ad_disable_tp === false) {
                        $buffy .= ' || ( ud_screen_width >= 768  && ud_screen_width < 1019 ) ';
                    }

                    if ($ud_ad_disable_p === false) {
                        $buffy .= ' || ( ud_screen_width < 768 ) ';
                    }
                $buffy .= ';'."\n";
                $buffy .= 'if(!'.$ad_array['is_visible_variable_name'].'){'."\n";
                $buffy .= 'document.getElementsByClassName("td-a-rec-id-' . $spot_id . $align . '")[0].remove();'."\n";
                $buffy .= '}'. "\n";
                $buffy .= '</script>' . "\n";
                $buffy .= do_shortcode( stripslashes( $ad_array['ud_real_ad_code'] ) );
            }else {
