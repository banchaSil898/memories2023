...
class td_video_support{
	...
	static function render_video($videoUrl) {
		...
		switch (self::detect_video_service($videoUrl)) {
			...
			//Unixdev MOD: support Unixdev Player -------------
            case 'udplayer':
                $buffy = do_shortcode( shortcode_unautop( $videoUrl ) );
                break;
            //-------------------------------------------------
		}
		return $buffy;
	...
	private static function validate_video_url($videoUrl) {
		...
		//Unixdev MOD: Unixdev Video Player support
		if (!preg_match('|^http(s)?://[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(/.*)?$|i', $videoUrl)
            && !preg_match('|^\[udplayer.*\]$|i', $videoUrl)) {
			return false;
		}
		//-----------------------------------------
		return true;
    ...
    private static function detect_video_service($videoUrl) {
		...
		//Unixdev MOD: support Unixdev Video Player ---------------
        if (strpos($videoUrl,'udplayer') !== false) {
            return 'udplayer';
        }
        //---------------------------
