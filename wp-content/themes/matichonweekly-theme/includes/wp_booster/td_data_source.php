...
class td_data_source {
    ...
    static function shortcode_to_args($atts = '', $paged = '') {
        ...
        extract(shortcode_atts(
                array(
                    ...
                    //Unixdev MOD ------
                    'ud_taxonomy_name' => '',
                    'ud_taxonomy_term_include_ids' => '',
                    'ud_taxonomy_term_exclude_ids' => '',
                    //------------------
	    ...
	    */
        ...
        //Unixdev MOD ----
        if ( ! empty( $ud_taxonomy_name ) and ( ! empty( $ud_taxonomy_term_include_ids ) or ! empty( $ud_taxonomy_term_exclude_ids ) ) ) {
            $query_params = array();

            if ( ! empty( $ud_taxonomy_term_include_ids )) {
                array_push($query_params , array(
                    'taxonomy' => $ud_taxonomy_name,
                    'field' => 'term_id',
                    'terms' => $ud_taxonomy_term_include_ids,
                ));
            }

            if ( ! empty( $ud_taxonomy_term_exclude_ids )) {
                array_push($query_params, array(
                    'taxonomy' => $ud_taxonomy_name,
                    'field' => 'term_id',
                    'terms' => $ud_taxonomy_term_exclude_ids,
                    'operator' => 'NOT IN',
                ));
            }

            if ( 2 === count($query_params)){
                $query_params['relation'] = "AND";
            }

            if (! empty($query_params)) {
                $wp_query_args['tax_query'] = $query_params;
            }
        }
        //----------------
        ...
        //Unixdev MOD: disable using SQL_CALC_FOUND_ROWS ----------
        if ( empty( $ajax_pagination ) ) {
            $wp_query_args['no_found_rows'] = true;
        }
        //---------------------------------------------------------

        //Unixdev MOD: enable sticky post manager query ----------
        if ( td_global::$is_ud_sticky_post_manager_activated ) {
            $udspm_loop_ids = array();
            if ( empty( trim( $ud_block_id_for_ad ) ) ) {
                foreach ( explode( ',', $category_ids ) as $cat_id ) {
                    //find first category filter id that is not nagative
                    if ( $cat_id > 0 ) {
                        $curr_cat = get_category( $cat_id );
                        if ( ! empty( $curr_cat ) and ! is_wp_error( $curr_cat ) ) {
                            array_push($udspm_loop_ids, $curr_cat->slug . '-block');
                        }
                    }
                }
            } else {
                $udspm_loop_ids = explode( ',', trim( $ud_block_id_for_ad ));
            }

            $wp_query_args['udspm'] = array(
                'loop_ids' => $udspm_loop_ids
            );
        }
        //---------------------------------------------------------

        //Unixdev MOD: query by taxonomy
        if ( ! empty( $ud_taxonomy_name ) and ( ! empty( $ud_taxonomy_term_include_ids ) or ! empty( $ud_taxonomy_term_exclude_ids ) ) ) {
            $query_params = array();
        ...
        switch ($sort) {
            ...
            case 'popular7':
                $wp_query_args['meta_query'] = 	array(
                    ...
                    array(
                        ...
                        //Unixdev MOD: round date('U') to five minute, use with query caching on database
                        'value'   => ( date( 'U' ) - ( date( 'U' ) % 300 ) - 604800 ), // current date minus 7 days
                        'type'    => 'numeric',
                        'compare' => '>'
                    )
                );
            ...
            case 'random_7_day':
                ...
                $wp_query_args['date_query'] = array(
                            //Unixdev MOD: post_date_gmt not indexed use post_date instead
                            // 'column' => 'post_date_gmt',
                            'after' => '1 week ago'
                            );
                break;
            case 'udpvc_count_1day':
                $wp_query_args['udpvc_orderby'] = array(
                    'count_1day' => 'DESC',
                    'post_id'    => 'DESC'
                );
                break;
            case 'udpvc_count_7day':
                $wp_query_args['udpvc_orderby'] = array(
                    'count_7day' => 'DESC',
                    'post_id'    => 'DESC'
                );
                break;
            case 'udpvc_count_total':
                $wp_query_args['udpvc_orderby'] = array(
                    'count_total' => 'DESC',
                    'post_id'     => 'DESC'
                );
        ...
        if (!empty($installed_post_types)) {
            ...
            //Unixdev MOD: detect if 'attachment' in array, set status to 'inherit' and ignore other post_type
            if (in_array('attachment', $array_selected_post_types)) {
                $wp_query_args['post_type'] = array('attachment');
                $wp_query_args['post_status'] = 'inherit';
            }
        }
    ...
    static function metabox_to_args($td_homepage_loop_filter, $paged = '') {
        ...
        //Unixdev MOD: force using SQL_CALC_FOUND_ROWS ----------
        if ( empty( $ajax_pagination ) ) {
