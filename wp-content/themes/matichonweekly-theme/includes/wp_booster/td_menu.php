...
class td_menu {
    ...
    // var $is_header_menu_mobile = true; //Unixdev MOD: split header-menu and mobile-menu, so not used these code ------
    ...
    function hook_wp_update_nav_menu_item ($menu_id, $menu_item_db_id, $args) {
        ...
        if ( isset( $_POST['ud_mobile_menu_default_uncollapse'][ $menu_item_db_id ] ) ) {
            update_post_meta( $menu_item_db_id, 'ud_mobile_menu_default_uncollapse', $_POST['ud_mobile_menu_default_uncollapse'][ $menu_item_db_id ] );
        }
        //-------------------------------------------------------------------

        //Unixdev MOD: custom onclick handler -------------------------------
        if ( isset( $_POST['ud_custom_onclick_handler'][ $menu_item_db_id ] ) ) {
            update_post_meta( $menu_item_db_id, 'ud_custom_onclick_handler', sanitize_textarea_field( $_POST['ud_custom_onclick_handler'][ $menu_item_db_id ] ) );
        }
        //-------------------------------------------------------------------
    ...
    function hook_wp_nav_menu_objects($items, $args = '') {
        ...
        foreach ($items as &$item) {
            ...
            //Unixdev MOD: split header-menu and mobile-menu
        ...
        //Unixdev MOD: split header-menu and mobile-menu, so not used these code ------
    ...
    function hook_init() {
        register_nav_menus(
            array(
                ...
                'mobile-menu' => 'Mobile Menu (By Unixdev)',
...
class td_tagdiv_walker_nav_menu extends Walker_Nav_Menu {
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( ! empty( $ud_custom_onclick_handler ) ) {
            $atts['onclick'] = $ud_custom_onclick_handler;
        }
        //-------------------------------------------------------------------
...
class td_walker_mobile_menu extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( in_array( 'menu-item-has-children', $classes ) && 'enable' === $ud_mobile_menu_default_uncollapse ) {
            $classes[] = 'td-sub-menu-open';
        }
        //-------------------------------------------------------------------
