...
//            echo '<br>'.print_r($count_7_day_array);
        //----------------------
    }
    ...
    static function on_manage_posts_custom_column($column_name, $id) {
        if($column_name === 'td_post_views'){
            //Unixdev MOD: should use $id instead of get_the_ID()
            echo self::get_page_views($id);
        }
    }

    // Unixdev MOD
    static function on_manage_posts_sortable_column_views($defaults){
        $defaults['td_post_views'] = array('td_post_views',1);
        return $defaults;
    }

    //Unixdev MOD
    static function request_sort_page_views( $vars ) {
        if ( isset( $vars['orderby'] ) && 'td_post_views' == $vars['orderby'] ) {
            $vars = array_merge( $vars, array(
                'meta_key' => self::$post_view_counter_key,
    ...
    //Unixdev MOD: atomic increment page view count, if not exist, set it to $default
    static function ud_atomic_incr_decr_meta_key($post_id, $meta_key, $count){
        global $wpdb;
        ...
        $set_str = "meta_value = meta_value + %d";

        if( $count < 0 ) {
            $set_str = "meta_value = meta_value - %d";
        }

        $sql = $wpdb->prepare("UPDATE `$table` SET $set_str  WHERE meta_key = %s AND post_id = %d", $count, $meta_key, $post_id);
