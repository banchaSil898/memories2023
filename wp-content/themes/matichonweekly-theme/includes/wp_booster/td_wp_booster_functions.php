...
require_once('ud_article_reference.php'); //Unixdev MOD: ud_article_reference
...
//Unixdev MOD: disable ajax search
// ajax: site wide search
if("yes" !== td_util::get_option( 'ud_disable_ajax_search' )){
...
function load_front_css() {
	...
	if (TD_DEBUG_USE_LESS) {
		...
		wp_enqueue_style('td-theme', get_stylesheet_uri(), '', filemtime( get_stylesheet_directory() . '/style.css'), 'all' );
...
function td_load_css_fonts() {
	...
	//Unixdev MOD: don't pull google font when not used-------------
    if (!empty($cur_td_fonts)) {
...
function load_front_js() {
	...
	if ($td_deploy_mode == 'dev') {
        ...
        wp_enqueue_script('td-site-min', td_global::$get_template_directory_uri . '/js/tagdiv_theme.min.js', array('jquery'), filemtime(td_global::$get_template_directory . '/js/tagdiv_theme.min.js'), true);
    }
...
function load_wp_admin_js() {
    ...
    if (TD_DEPLOY_MODE == 'dev') {
        ...
        wp_enqueue_script('td-wp-admin-js', td_global::$get_template_directory_uri . '/js/td_wp_admin.min.js', array('jquery', 'wp-color-picker', 'jquery-ui-sortable', 'jquery-ui-autocomplete'), TD_THEME_VERSION, false);
    }
    ...
    if (isset($_GET['page']) && $_GET['page'] === 'td_theme_panel') {
        ...
        foreach (td_global::$js_files_for_td_theme_panel as $js_file_id => $js_file_params) {
            if ($last_js_file_id == '') {
                wp_enqueue_script($js_file_id, td_global::$get_template_directory_uri . $js_file_params, array('jquery', 'wp-color-picker', 'jquery-ui-sortable', 'jquery-ui-autocomplete'), TD_THEME_VERSION, false); //first, load it with jQuery dependency
            } else {
                wp_enqueue_script($js_file_id, td_global::$get_template_directory_uri . $js_file_params, array($last_js_file_id), TD_THEME_VERSION, false);  //not first - load with the last file dependency
            }
            $last_js_file_id = $js_file_id;
...
function hook_wp_head() {
	if (is_single()) {
		...
		//Unixdev MOD: og:image will insert by using ud-social plugins
	...
	if(td_util::get_option('tds_ajax_post_view_count') == 'enabled') {
		...
		if(is_single()) {
			...
			if($post->ID > 0) {
                ...
                //Unixdev MOD: sampling view count 1 in max_value
                $ud_max_value = td_util::get_option( 'ud_sampling_max_value' );
                if ( ! empty( $ud_max_value ) && is_numeric( $ud_max_value ) && intval( $ud_max_value ) > 1 ) {
                    $ud_max_value = intval($ud_max_value);
                }else {
                    $ud_max_value = 1;
                }
                //--------------
                ...
                jQuery().ready(function jQuery_ready() {
                    tdAjaxCount.tdGetViewsCountsAjax("post",' . json_encode('[' . $post->ID . ']') . ', '. $ud_max_value .');
                });
            ');
			}
		}
...
/*  ----------------------------------------------------------------------------
    Custom head ( Unixdev )
 */
add_action('wp_head', 'ud_header_custom_code', 40);
function ud_header_custom_code() {
    // Unixdev MOD: custom css in head tag -----
	$uds_head_custom_css = stripslashes( td_util::get_option( 'uds_head_custom_css' ) );

	if ( ! empty( $uds_head_custom_css ) ) {
		$css_buffy = PHP_EOL . '<!-- Custom css form theme panel -->';
		$css_buffy .= PHP_EOL . '<style type="text/css" media="screen">';
		$css_buffy .= PHP_EOL . '/* custom css theme panel */' . PHP_EOL;
		$css_buffy .= $uds_head_custom_css . PHP_EOL;
		$css_buffy .= '</style>' . PHP_EOL . PHP_EOL;
	...
	// Unixdev MOD: custom javascript in head tag -----
	$uds_head_custom_javascript = stripslashes( td_util::get_option( 'uds_head_custom_javascript' ) );
	if ( ! empty( $uds_head_custom_javascript ) ) {
		echo '<script type="text/javascript">' . PHP_EOL . $uds_head_custom_javascript . PHP_EOL . '</script>';
	}
	//--------------------------------------------------------

    // Unixdev MOD: custom HTML in head tag -----
    $uds_head_custom_html = stripslashes( td_util::get_option( 'uds_head_custom_html' ) );
    if ( ! empty( $uds_head_custom_html ) ) {
        echo $uds_head_custom_html;
    }
    //--------------------------------------------------------
}
...
function td_gallery_shortcode($output = '', $atts, $content = false) {
	...
	$page_template_slug = get_page_template_slug($post); //Unixdev MOD: prevent warning when $post is null
...
/* ----------------------------------------------------------------------------
 * Unixdev MOD: add unixdev sticky post manager loop_id
 */
add_action( 'pre_get_posts', 'ud_add_sticky_post_loop_ids' );
function ud_add_sticky_post_loop_ids( $query ) {
    ...
    if ( is_category() ) {
        ...
        $query->set( 'udspm', array(
            'loop_ids' => array(
                td_global::$current_category_obj->slug,
                '__all-category'
            )
        ));
    }
