...
class td_ajax {
	...
	static function on_ajax_update_views () {
		...
		if (!empty($_POST['td_post_ids'])) {
            ...
            //Unixdev MOD: ajax view count sampling
            ...
            if ( ! empty( $ud_max_value ) && is_numeric( $ud_max_value ) && intval( $ud_max_value ) > 1 ) {
                $ud_max_value_from_js = sanitize_text_field( $_POST['ud_sampling_max_value'] );
                if ( ! empty( $ud_max_value_from_js ) && is_numeric( $ud_max_value_from_js ) && intval( $ud_max_value_from_js ) === intval( $ud_max_value ) ) {

                    $new_post_count = $current_post_count + $ud_max_value;
                } else {
                    die( json_encode( array( $td_post_id[0] => $current_post_count ) ) );
                }
            }
            //---------------
