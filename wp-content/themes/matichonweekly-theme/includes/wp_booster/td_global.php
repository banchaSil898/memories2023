...
class td_global {
    ...
    //Unixdev MOD --------------------
    static $ud_force_author = ''; //Unixdev MOD: Force author (WP_user|false)
    static $is_ud_columnist_manager_activated = false; //Unixdev MOD: Is Unixdev Columnist Manager plugin activated (true|false)
    static $is_ud_sticky_post_manager_activated = false; //Unixdev MOD: Is Unixdev Sticky Post Manager plugin activated (true|false)
    static $is_ud_post_view_counter_activated = false; //Unixdev MOD: Is Unixdev Post View Counter plugin activated (true|false)
    //--------------------------------

    //Unixdev MOD: ads spot id to use throughout current page
    static $ud_header_ad_spot_id = '';
    static $ud_footer_ad_spot_id = '';
    static $ud_takeover_ad_spot_id = '';
    static $ud_under_menu_ad_spot_id = '';
    //------------
    ...
    static function load_single_post($post) {
        ...
        if (is_single()) {
            ...
            //Unixdev MOD: if post is in multiple category try getting from url, for using in breadcrumb and other
            $category_from_query = get_category_by_path(get_query_var( 'category_name' ),false);
            if ( ! empty ( $category_from_query ) and ! is_wp_error($category_from_query) and 'hide' !== td_util::get_category_option( $category_from_query->term_id, 'tdc_hide_on_post' )) {
                self::$primary_category = $category_from_query->term_id;

                return;
            }
            //-----------------------------------------

            //Unixdev MOD: try getting from yoast wordpress seo primary category
            if ( class_exists( 'WPSEO_Primary_Term' ) ) {
                $primary_term_object = new WPSEO_Primary_Term( 'category', self::$post->ID );
                $cat_id = $primary_term_object->get_primary_term();
                $category = get_category( $cat_id );

                if ( ! empty( $category ) and ! is_wp_error( $category ) and $category->name != TD_FEATURED_CAT and 'hide' !== td_util::get_category_option( $cat_id, 'tdc_hide_on_post' )
                ) {
                    self::$primary_category = $cat_id;
            ...
            foreach($categories as $category) {
                //Unixdev MOD: hide feature and hide category from tdc_hide_on_post option
                if ($category->name != TD_FEATURED_CAT and 'hide' !== td_util::get_category_option( $category->cat_ID, 'tdc_hide_on_post' )) { //ignore the featured category name and hide category
...
if( class_exists( 'UDColumnistManager\ColumnistManager' ) ) {
    td_global::$is_ud_columnist_manager_activated = true;
}

if( class_exists( 'UDStickyPostManager\UDStickyPostManager' ) ) {
    td_global::$is_ud_sticky_post_manager_activated = true;
}

if( class_exists( 'UDPostViewCounter\UDPostViewCounter' ) ) {
    td_global::$is_ud_post_view_counter_activated = true;
}
//---------------------------
