...
class td_api_ad extends td_api_base {
	...
	//Unixdev MOD: add $selected_ad_ids to allow manual specifying ad id list to display
    //example $selected_ad_ids = ["header","footer"]
	static function helper_display_ads($selected_ad_ids=null) {

        //Unixdev MOD ------------------------------
        if ( ! is_null( $selected_ad_ids ) && is_array( $selected_ad_ids ) ) {
            $tmp_all_ads = td_api_ad::get_all();
            $ud_all_ads = array();
            foreach ( $selected_ad_ids as $ad_id ) {
                if( !empty($tmp_all_ads[$ad_id])) {
                    $ud_all_ads[$ad_id] = $tmp_all_ads[$ad_id];
                }
            }
        }else {
            $ud_all_ads = td_api_ad::get_all();
        }
        //-------------------------------------------

        //Unixdev MOD: use $ud_all_ads instead
		foreach ($ud_all_ads as $ad_id => $ad_array) {
