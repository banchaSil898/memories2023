...
function td_js_generator() {
    ...
    if(!preg_match('/^#[a-f0-9]{6}$/i', $td_get_db_theme_color)) {
        $td_get_db_theme_color = '#dd001a';//default theme color //Unixdev MOD: loader animation color
    }
    ...
    td_js_buffer::add_variable('td_magnific_popup_translation_tCounter', __td('%curr%/%total%', TD_THEME_NAME));
    ...
    if (td_util::get_option('tds_data_js') == 'true') {
        ...
        if ($td_date_i18n_format == '') {
            $td_date_i18n_format = get_option('date_format');
        }
