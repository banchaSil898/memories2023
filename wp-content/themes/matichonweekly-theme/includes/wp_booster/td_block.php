...
class td_block {
    ...
    function render($atts, $content = null) {
		...
		//Unixdev MOD: td_column_number dropdown "None" is an empty string, not an undefined empty,
		//so default value is not set by shortcode_atts.
		if ( empty( $atts['td_column_number'] ) ) {
			$atts['td_column_number'] = td_global::vc_get_column_number();
		}
		//---------------------------
	    ...
	    $this->atts = shortcode_atts ( // add defaults (if an att is not in this list, it will be removed!)
		    array(
                ...
                // Unixdev parameter
                'ud_block_adspot_bottom_id'    => '',   //Unixdev MOD
                'ud_block_adspot_top_id'       => '',   //Unixdev MOD
                'ud_taxonomy_name'             => '',   //Unixdev MOD
                'ud_taxonomy_term_include_ids' => '',   //Unixdev MOD
                'ud_taxonomy_term_exclude_ids' => '',   //Unixdev MOD
                'ud_use_category_url'          => '',   //Unixdev MOD
                'ud_block_id_for_ad'           => '',   //Unixdev MOD
                'ud_readmore_text'             => '',   //Unixdev MOD
                'ud_readmore_url'              => '',   //Unixdev MOD
            ),
		    $atts
	    );
...
//					}
    ...
    function get_block_title() {
		// Unixdev MOD
		$ud_adspot = '';
		$ud_block_adspot_top_id = $this->get_att( 'ud_block_adspot_top_id' );
		if ( ! empty( $ud_block_adspot_top_id ) && td_util::is_ad_spot_enabled( $ud_block_adspot_top_id ) ) {
			$ud_adspot = td_global_blocks::get_instance( 'td_block_ad_box' )->render( array( 'spot_id' => $ud_block_adspot_top_id ) );
		}
		//-------------

        return $this->block_template()->get_block_title().$ud_adspot;
    }
    ...
    function get_block_pagination() {
        ...
        //Unixdev MOD
//		$ud_block_adspot_bottom_id = $this->get_att( 'ud_block_adspot_bottom_id' );
//		if ( ! empty( $ud_block_adspot_bottom_id ) && td_util::is_ad_spot_enabled( $ud_block_adspot_bottom_id ) ) {
//			$ud_adspot = td_global_blocks::get_instance( 'td_block_ad_box' )->render( array( 'spot_id' => $ud_block_adspot_bottom_id ) );
//			$buffy .= $ud_adspot;
//		}
		//-------------
    ...
    function ud_get_readmore() {
        $readmore_url = $this->get_att('ud_readmore_url');
        $readmore_text = $this->get_att('ud_readmore_text');
        ...
        // button text
        if (empty($readmore_url)) {
            $readmore_url = get_category_link($category_id);
            if (empty($readmore_url)) {
                return '';
            }
        }

        $text_buffy = esc_html($readmore_text);

        $buffy = '';
        $buffy .= '<a href="' . esc_url( $readmore_url ) . '" data-td_block_id="' . $this->block_uid . '">';
        $buffy .= $text_buffy;
        $buffy .= '</a>';
    ...
    protected function get_block_classes($additional_classes_array = array()) {
        ...
        //Unixdev MOD: mark ud_block_with_readmore class for styling block according to block option
        $ud_readmore_url = $this->get_att( 'ud_readmore_url' );
        $ud_readmore_text = $this->get_att( 'ud_readmore_text' );
        $ud_category_id = $this->get_att( 'category_id' );

        if ( ! empty( $ud_readmore_text ) and ( ! empty( $ud_readmore_url ) or ! empty( get_category_link( $ud_category_id ) ) ) ) {
            array_push( $block_classes, 'ud_block_with_readmore' );
        }
        //-------------
