...
function td_register_post_metaboxes() {
    ...
    // Unixdev MOD: Article Reference
    new WPAlchemy_MetaBox(array(
        'id' => 'ud_article_references',
        'title' => 'Article Reference',
        'types' => array('post'),
        'priority' => 'low',
        'context' => 'side',
        'template' => $td_template_settings_path . 'ud_article_reference_settings.php',
        'save_filter' => array('ud_article_reference', 'on_save_post_article_reference'),
    ));
    ...
    // Unixdev MOD: remove the ud-book post type from the array if it's available and the woo_commerce plugin is installed
    $ud_book_key = array_search( UDBook::POST_TYPE_NAME, $td_custom_post_types );
    if ( $ud_book_key !== false ) {
        unset( $td_custom_post_types[ $ud_book_key ] );
    }

    //Unixdev MOD: set metabox for ud-book
    if ( current_user_can( 'publish_posts' ) ) {
        new WPAlchemy_MetaBox( array(
            'id'       => 'td_post_theme_settings',
            'title'    => 'Book settings',
            'types'    => array( UDBook::POST_TYPE_NAME,),
            'priority' => 'high',
            'template' => get_template_directory() . '/includes/wp_booster/wp-admin/content-metaboxes/td_set_post_settings_ud_book.php',
        ) );
    }
