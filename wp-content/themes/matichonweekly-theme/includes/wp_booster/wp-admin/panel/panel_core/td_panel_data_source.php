        ...
        foreach($wp_option_array as $box_add => $values){
            if(!empty($values['ad_code'])) {
                ...
                if(preg_match('/googlesyndication.com/', $ad_code)){
                    ...
                    preg_match( '/^.*?(\r\n|\r|\n)/m', $ad_code, $match );
                    $ud_newline_char = ! empty( $match ) ? $match[1] : "\n";
                    $ud_var_name = $box_add . '_script_ads_show';
                    $wp_option_array[ $box_add ]['is_visible_variable_name'] = $ud_var_name;
                    // insert if statement
                    $ud_real_ad_code = preg_replace_callback( '#(\<script(.*?)\>)(.*?)(\</script\>)#s',
                        function ( $matches ) use ( $ud_var_name, $ud_newline_char ) {
                            // check if already contain if statement
                            if ( ! preg_match( '/' . $ud_var_name . '/s', $matches[3] ) ) {
                                $buffy = $matches[1] . $ud_newline_char;
                                $buffy .= '// if statement below is generated by theme, "' . $ud_var_name . '" will defined at runtime according to device disable/enable option below.' . $ud_newline_char;
                                $buffy .= 'if(' . $ud_var_name . '){' . $ud_newline_char;
                                $buffy .= $matches[3];
                                $buffy .= $ud_newline_char . '}' . $ud_newline_char . $matches[4];
                    ...
                    if ( $ud_real_ad_code !== $wp_option_array[ $box_add ]['ad_code'] ) {
                        $wp_option_array[ $box_add ]['ud_real_ad_code'] = $ud_real_ad_code;
                    }
